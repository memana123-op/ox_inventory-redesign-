...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if not toSlot then
		...
		-- auto-placement never targets reserved equipment slots
		local maxSlot = (inv.player and shared.equipmentBaseSlot) or inv.slots
...
local function dropItem(source, playerInventory, fromData, data)
	...
	Inventory.Drops[dropId] = {coords = inventory.coords, instance = data.instance, model = require('modules.custom.shared').getDropModel(toData.name)}
...
function Inventory.GetEmptySlot(inv)
	...
	local maxSlot = (inventory.player and shared.equipmentBaseSlot) or inventory.slots
