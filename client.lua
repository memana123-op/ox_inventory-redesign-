...
local function registerCommands()
	...
	-- Keys 1-2 draw the equipped weapons (equipment slots), 3-8 use the
	-- quick-use row (inventory slots 1-6).
	for i = 1, 8 do
		lib.addKeybind({
			...
			onPressed = function()
				...
				if i <= 2 then
					return useSlot(shared.equipmentBaseSlot and (shared.equipmentBaseSlot + i) or i)
				end
...
local CustomDrops = require 'modules.custom.drops.client'

local function createDrop(dropId, data)
	if CustomDrops.enabled then
		client.drops[dropId] = CustomDrops.create(dropId, data)
		return
	end
...
RegisterNetEvent('ox_inventory:removeDrop', function(dropId)
	if client.drops then
		...
		if point then
			...
			if CustomDrops.enabled then CustomDrops.onRemoved(point) end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	for _, v in pairs(Items --[[@as table<string, OxClientItem>]]) do
		...
		ItemData[v.name] = {
			...
			rarity = Custom.getRarity(v.name, v.rarity, v.weapon ~= nil)
		}
	end
	...
	SendNUIMessage({
		...
		data = {
			...
			equipment = shared.equipmentBaseSlot and {
				baseSlot = shared.equipmentBaseSlot,
				slots = shared.custom.equipment.slots,
			} or nil,
			components = (shared.custom.components and shared.custom.components.enabled) and {
				slotTypes = shared.custom.components.slotTypes,
				excludeWeapons = shared.custom.components.excludeWeapons,
			} or nil,
...
-- Extended systems: clothing sync, frisking, hold/throw/place
require 'modules.custom.client'

-- Cinematic camera, DOF and player info header while the inventory is open
require 'modules.custom.cinematic.client'
