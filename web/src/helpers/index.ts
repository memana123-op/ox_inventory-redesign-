...
// Rarity tier for an item: per-instance metadata wins, then the tier resolved
// server-side from data/custom.lua (sent with the item definitions). Returns
// undefined for common items so callers can skip badges/stripes entirely.
export const getItemRarity = (item: {
  name?: string;
  metadata?: { rarity?: string };
}): string | undefined => {
  const rarity = item.metadata?.rarity || (item.name ? Items[item.name]?.rarity : undefined);
  if (!rarity) return undefined;

  const tier = String(rarity).toLowerCase();
  return tier === 'common' ? undefined : tier;
};
