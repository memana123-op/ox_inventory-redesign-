...
>(({ label, disabled, icon, danger, hint, ...props }, forwardedRef) => {
  ...
  return (
    ...
    >
      {icon && <span className="cmi-icon">{icon}</span>}
      <span className="cmi-label">{label}</span>
      {hint && <span className="cmi-hint">{hint}</span>}
    </button>
  );
});

// Non-interactive separator between action groups. Rendered as a plain div so
// it never participates in the menu's list navigation.
export const MenuDivider: React.FC = () => <div className="context-menu-divider" role="separator" />;
